    ...
    return (
        <section className="py-24 bg-[#050505] relative overflow-hidden font-sans">
            ...
            <div className="container mx-auto px-6 lg:px-12 relative z-10">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
                    <div>
                        ...
                        <h2 className="text-4xl md:text-5xl font-bold text-white tracking-tight leading-[1.1]">
                            ...
                            <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-teal-500"> Workforce Excellence.</span>
                        </h2>
                    </div>
                ...
                {/* THE BENTO GRID */}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 min-h-[600px] md:h-[500px]">
                        ...
                        >
                            ...
                            <div className="relative z-10">
                                <div className="flex items-start justify-between mb-6">
                                    <div className={`p-3 rounded-2xl bg-white/5 border border-white/5 ${item.accent}`}>
                                        <item.icon size={28} strokeWidth={1.5} />
                                    </div>
                                    <ArrowUpRight className={`opacity-0 -translate-x-2 translate-y-2 group-hover:opacity-100 group-hover:translate-x-0 group-hover:translate-y-0 transition-all duration-300 ${item.accent}`} />
                                </div>
                            ...
                            {/* Bottom Part: Stats */}
                            <div className="relative z-10 mt-8 pt-6 border-t border-white/5 flex items-center gap-8">
                                {item.stats.map((stat, idx) => (
                                    <div key={idx}>
                                        <div className="text-[10px] uppercase tracking-widest text-white/30 mb-1">{stat.label}</div>
                                        <div className={`text-xl font-mono font-medium ${item.accent}`}>{stat.val}</div>
                                    </div>
                                ))}
                            </div>
